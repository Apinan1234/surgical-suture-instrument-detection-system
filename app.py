...
class DetectionTab(ttk.Frame):
    ...
    def _build_left(self, p):
        ...
        self._model_var = tk.StringVar(value="models/best.pt")
        mr = tk.Frame(sm, bg=PANEL_BG); mr.pack(fill="x", pady=(0,4))
...
class LiveVerifierTab(ttk.Frame):
    ...
    def _toggle_cam(self):
        if self._running:
            ...
            if not self.app._model_var.get():
                messagebox.showerror("Error", "Please select a model in Step 2 first.")
                return
            
            if not self._detector:
                self._detector = YOLOv11Detector(self.app._model_var.get())
